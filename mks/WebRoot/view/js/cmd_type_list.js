...
function update(id){
	openEditorDialog(_updateTitle, _dialogWidth, _dialogHeight, _updateAction+"?id="+id);
}
function look(value){
	  rezDialog('认证', _dialogWidth, _dialogHeight, _updateAction+"?uid="+id+"&code=1");
}
$(function(){
	  $('#tableBox').datagrid({
		    rownumbers:true,
...
function rezDialog(dialogTitle, dialogWidth, dialogHeight, url){
	$("#openDialogDiv").dialog({
		title:dialogTitle,
		href: url,
		closed:false,
