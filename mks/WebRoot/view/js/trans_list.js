$(function() {
	...
	$("#btnTrans").click(function(){
		...
		openEditorDialog(_addTitle, _dialogWidth, _dialogHeight, _addAction+"?inventoryId="+uid);
...
$('#trangrid').pagination.defaults.showPageList = false;
function loadPagination() {
	...
	var p2 = $('#trangrid').datagrid('getPager');
	if (p2) {
		$(p2).pagination({
			pageSize:25,
			beforePageText:'第',
...
var transTypes = {"1":"入库","2":"出库"};
function getTransType(val,row){
...
function editStock(){
	...
	if (selects.length==1) {
		...
		$.ajax({
			type:"post",
			dataType:"json",
			url:"${path}/rep/inventory/editStock.do",
			data:{
				inventoryId:uid,
				stock:num
			},
...
function showTrans(){
	var queryParams = $('#trangrid').datagrid('options').queryParams;
	var selects =  $('#dbgrid').datagrid('getSelections');
	if (selects.length==1) {
		var ids = selects[0][getId()];
		queryParams.inventoryId = ids;
	}
